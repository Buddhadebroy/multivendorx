...
window.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.multivendorx-contact-info-wrapper').forEach(wrapper => {
    const hideFromGuests = wrapper.dataset.hideFromGuests === 'true';
    const enableGoogleRecaptcha = wrapper.dataset.enableGoogleRecaptcha === 'true';
    const googleRecaptchaType = wrapper.dataset.googleRecaptchaType || 'v2';
    const recaptchaV2Scripts = wrapper.dataset.recaptchaV2Scripts || '';
    const recaptchaV3Sitekey = wrapper.dataset.recaptchaV3Sitekey || '';
    const recaptchaV3Secretkey = wrapper.dataset.recaptchaV3Secretkey || '';
    const root = wrapper.querySelector('.multivendorx-contact-info');
    if (!root) {
      return;
    }
    root.innerHTML = `
				...
				>
					...
					${enableGoogleRecaptcha ? `
							...
							>
								...
								${googleRecaptchaType === 'v2' && recaptchaV2Scripts ? `
