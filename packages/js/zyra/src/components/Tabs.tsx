...
const Tabs: React.FC<TabsProps> = ({
    ...
    const renderBreadcrumb = useCallback(() => breadcrumbPath.map((crumb, index) => (
        <span key={index}>
            {index > 0 && ' / '}
            <Link to={crumb.type === 'file' ? prepareUrl(crumb.id) : '#'} onClick={(e) => { e.preventDefault(); goToBreadcrumb(index); }}>
                {crumb.name}
            </Link>
        </span>
    ...
    const renderMenuItem = useCallback((item: TabData, index: number) => {
        ...
        if (item.type === 'folder') {
            ...
            return (
                <div key={`folder-${folderName}-${index}`} className={`menu-item ${isActiveFolder ? 'active-current-tab' : ''}`} onClick={() => openSubmenu(folderName, folderContent)}>
                    <p className="menu-name">{menuCol ? null : folderName}</p>
                    {!menuCol && <span className="submenu-arrow"><i className="admin-font adminlib-keyboard-arrow-right"></i></span>}
                </div>
            );
        }
        return null;
